...
Desplegado con:
  - Motor RAG: NVIDIA Build (build.nvidia.com) con modelo meta/llama-3.1-70b-instruct
...
if "motor_rag" not in st.session_state:
    with st.spinner("🚀 Inicializando Agente RAG e ingestando documentos del repositorio..."):
        # Inicializa con NVIDIA Build (meta/llama-3.1-70b-instruct) por defecto
...
with st.sidebar:
    ...
    st.markdown('<div class="status-badge">🟢 Modelo: NVIDIA Build (meta/llama-3.1-70b-instruct)</div>', unsafe_allow_html=True)
...
st.caption(
    ...
    "procesados del repositorio mediante RAG con **NVIDIA Build (meta/llama-3.1-70b-instruct)**."
)
...
if prompt := st.chat_input("Escribe tu pregunta sobre políticas de RH, soporte, legal o procesos corporativos..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("🔍 Consultando base de conocimiento con NVIDIA Build (meta/llama-3.1-70b-instruct)..."):
            try:
...
if not st.session_state.mensajes:
    ...
    *Powered by **NVIDIA Build (meta/llama-3.1-70b-instruct)** + **RAG**.*
    """)